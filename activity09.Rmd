...
We've made R Code for a recursive implementation of the Conjecture. That is, the function will repeatedly call itself to eventually reduce $n$ to 1. Included is code to record the "Stopping Points", which are positive integers that state how many times the function is run to reduce $n$ to 1.

```{r Chunk1: CollatzCode}
library(groundhog)

groundhog.day="2023-11-20"
pkgs=c('janitor','dplyr','kableExtra', 'ggplot2')
...
```{r Chunk2: stoppingPointsHistogram}
...
```{r Chunk3: diamondsDotPlot1, message=FALSE, warning=FALSE}
...
```{r Chunk4: diamondsDotPlot2, message=FALSE, warning=FALSE}
...
```{r Chunk5: diamondsSummary, message=FALSE, warning=FALSE}
